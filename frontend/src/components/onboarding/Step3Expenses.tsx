...
import type { ExpenseItem, ExpenseSubtype, FormData } from './types';
import { FIXED_EXPENSES, VARIABLE_EXPENSES, DEBT_EXPENSES } from './constants';
import { submitExpense } from '../../api/onboarding.api';
...
const SECTION_CONFIG: {
...
}[] = [
  {
    ...
    subtitle:
      'House rent, EMIs, insurance — anything that stays the same each month.',
    subtype: 'fixed',
    chips: FIXED_EXPENSES,
  },
  {
    title: 'Variable Expenses (This Month)',
    subtitle: 'Food, travel, shopping — spending that changes every month.',
    subtype: 'variable',
    chips: VARIABLE_EXPENSES,
  },
  {
    title: 'Debts / Money You Owe (Due This Month)',
    subtitle: 'Enter the exact dues you plan to pay now.',
    subtype: 'one-time',
    chips: DEBT_EXPENSES,
  },
];

const subtypeLabel: Record<ExpenseSubtype, string> = {
  fixed: 'Fixed',
  variable: 'Variable',
  'one-time': 'One-time',
};
...
export default function Step3Expenses({
  ...
  const expenses: ExpenseItem[] = useMemo(
    () => (Array.isArray(formData.expenses) ? formData.expenses : []),
    [formData.expenses]
  );
  ...
  const [customSubtype, setCustomSubtype] = useState<ExpenseSubtype>('fixed');

  const handleSelect = (label: string, subtype: ExpenseSubtype) => {
    const alreadySelected = expenses.some(
      (expense) =>
        expense.category.toLowerCase() === label.toLowerCase() &&
        expense.subtype === subtype
    );
  ...
  const handleAddCustomExpense = () => {
    ...
    const alreadySelected = expenses.some(
      (expense) =>
        expense.category.toLowerCase() === trimmedName.toLowerCase() &&
        expense.subtype === customSubtype
    );
    if (alreadySelected) {
  ...
  const expensesBySubtype = (subtype: ExpenseSubtype) =>
    expenses.filter((expense) => expense.subtype === subtype);
  ...
  const canSubmit = expenses.length > 0 && allAmountsFilled && !isSubmitting;
    ...
    try {
      ...
      const message =
        (error as { response?: { data?: { message?: string } } })?.response
          ?.data?.message || 'Unable to save expenses. Please try again.';
  ...
  const renderSelectedExpenses = (subtype: ExpenseSubtype) => {
    const selected = expensesBySubtype(subtype);
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-text/60 mt-0.5">
                  {subtypeLabel[expense.subtype]} expense · This month only
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <label className="text-sm font-medium text-[#111827]">
                ...
                <input
                  ...
                  value={expense.note || ''}
                  onChange={(event) =>
                    handleFieldChange(expense.id, 'note', event.target.value)
                  }
                  placeholder="Example: Room rent / groceries / credit card bill"
      ...
      <div className="max-w-3xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-white/80 rounded-3xl p-8 shadow-soft border border-pastel-tan/20">
          <div className="mb-8">
            ...
            <p className="text-sm text-text/60">Step 3 of 5</p>
          </div>
          <div className="mb-8">
            ...
            <p className="text-text/70">
              Pick what you actually paid (or expect to pay) this month — no
              averages, no projections.
            </p>
          </div>
            ...
            <div key={section.subtype} className="mb-12">
              ...
              <div className="flex flex-wrap gap-3">
                {section.chips.map((chip) => {
                  const isSelected = expenses.some(
                    (expense) =>
                      expense.category === chip &&
                      expense.subtype === section.subtype
                  );
                  return (
                    <button
                      key={chip}
              ...
              {renderSelectedExpenses(section.subtype)}
            </div>
          ))}
          ...
          <div className="mb-12">
            ...
            <p className="text-sm text-text/60 mb-4">
              Add anything else you spent money on this month.
            </p>
            {showCustomForm ? (
              <div className="rounded-xl border border-[#E5E7EB] bg-[#F9FAFB] p-4">
                <label className="text-sm font-medium text-[#111827]">
                  ...
                  <input
                    ...
                    placeholder="Example: Pet care, Tuition, Repairs"
                    className="mt-1 w-full rounded-lg border border-[#E5E7EB] bg-white px-4 py-3 text-sm text-[#111827] focus:border-[pastel-green] focus:outline-none focus:ring-2 focus:ring-[pastel-green]/30"
                  />
                </label>
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(
                      ['fixed', 'variable', 'one-time'] as ExpenseSubtype[]
                    ).map((subtype) => (
                      <button
                        key={subtype}
                        type="button"
